...
int main(int argc, char *argv[]) {
    ...
    printf("平均気温が最も高い都市 %11s %11.3f %11.3f\n", data[max_p]->cell[0], atof(data[max_p]->cell[1]), atof(data[max_p]->cell[2]));
    printf("平均気温が最も低い都市 %11s %11.3f %11.3f\n", data[min_p]->cell[0], atof(data[min_p]->cell[1]), atof(data[min_p]->cell[2]));
    return 0;
}
#endif
