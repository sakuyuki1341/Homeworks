...
bool equal ( position p, position q ) { return ( p.i == q.i && p.j == q.j ); }
...
bool is_in ( maze *m, position p ) { return ( 0 <= p.i && p.i < m->size_row && 0 <= p.j && p.j < m->size_col ); }
bool is_goal ( maze *m, position p ) { return ( m->map [ p.i ][ p.j ] == 'G' ); }
bool is_wall ( maze *m, position p ) { return ( m->map [ p.i ][ p.j ] == '#' ); }
...
void solve ( maze *m )
{
  ...
  while(! is_goal ( m, cur ) ) { // ゴールに到達するまで繰り返し
    ...
    case 0: // 行き止まり
      printf("DEADEND\n");
      exit ( 1 ); // めちゃくちゃ行儀悪いけど説明の都合上こうする
      break;

    default:
      push ( &route, cur ); // 今いる位置を保持する
      cur = candidate [ 0 ]; // 次の位置へ移動
      break;
    }
  }
