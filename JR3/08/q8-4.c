...
void merge(struct point a[], int m, int n, int h) {
	...
	struct point arr[n-m+1];	//並べ替え後の配列
...
///
/// 配列aのm番目からn番目までを、マージソートアルゴリズムにより、
/// 指定された基準に整列する
///
void merge_sort(struct point a[], int m, int n) {
	if(m < n) {
		int h = (m+n)/2;	// 前半終了の添字
		merge_sort(a, m, h);	//m番目からh番目をマージソート
		merge_sort(a, h+1, n);	//h+1番目からn番目をマージソート
		merge(a, m, n, h);	//m番目からh番目までとh+1番目からn番目を併合
	}
}
